...
describe('Distributor', () => {
    ...
    it('should deploy', async () => {
        ...
        let ownerAddess = randomAddress()

        const distributor = blockchain.openContract(
            await Distributor.createFromConfig({
                owner: ownerAddess,
                processingPrice: toNano('0.05'),
                shares: [{address: randomAddress(), factor: 1, base: 1, comment: 'test'}],
                seed: 0
            }, code)
        )
        ...
        expect(ownerAddess.equals(owner)).toBe(true)
    })
    ...
    it('should distribute coins according to shares', async () => {
        ...
        const distributor = blockchain.openContract(
            Distributor.createFromConfig({
                ...
                seed: 0,
                shares: [
                    {address: randomAddress(), factor: 1, base: 2, comment: 'first half'},
                    {address: randomAddress(), factor: 1, base: 2, comment: 'second half'},
                ]
            }, code)
        )
